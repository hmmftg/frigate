...
$url = "https://hosted.weblate.org/api/translations/frigate-nvr/views-settings/fa/units/?format=json&page_size=100"
$allUnits = @()
while ($url) {
...
foreach ($u in $allUnits) {
    ...
    # Skip if local file still has English (proper nouns etc)
    ...
    try {
        ...
        $updated++
        if ($updated % 100 -eq 0) { Write-Output "Updated $updated units..." }
        Start-Sleep -Milliseconds 30
    } catch {
        $failed++
        Write-Output "FAILED: id=$($u.id) ctx=$ctx"
    }
}
