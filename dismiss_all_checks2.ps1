...
foreach ($comp in $components) {
    ...
    while ($url) {
        ...
        foreach ($u in $resp.results) {
            ...
            # Ensure URL has format=json (it already does from the API)
            $getUrl = $sourceUnitUrl
            if (-not $getUrl.Contains("format=json")) {
                $getUrl = $getUrl + "?format=json"
            }
            
            try {
                $sourceUnit = Invoke-RestMethod -Uri $getUrl -Headers $headers -Method Get
                $existingFlags = $sourceUnit.extra_flags
                
                # Add ignore flags if not already present
                $newFlags = $existingFlags
                if (-not $newFlags) { 
                    $newFlags = "ignore-inconsistent,ignore-unchanged" 
                }
                elseif (-not $newFlags.Contains("ignore-inconsistent")) { 
                    $newFlags = "$newFlags,ignore-inconsistent,ignore-unchanged" 
                }
                else { 
                    continue # Already has flags
                }
                
                $body = @{ extra_flags = $newFlags } | ConvertTo-Json
                Invoke-RestMethod -Uri $getUrl -Headers $headers -Method Patch -Body $body | Out-Null
                $processed++
