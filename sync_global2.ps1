...
# For each unit, check if Weblate target differs from local Persian file
# If Weblate still has English (source==target) but local file has Persian, update it
$updated = 0
$skipped = 0
$failed = 0
...
foreach ($u in $allUnits) {
    ...
    # Skip if local file still has English (untranslated)
    if ($persianVal -eq $enVal) { continue }
    
    # Get current Weblate target as string
    $weblateTarget = if ($u.target -is [array]) { $u.target -join "" } else { $u.target }
    
    # Skip if Weblate already has the correct Persian value
...
Write-Output "Done: updated=$updated skipped=$skipped failed=$failed"
